...
func TestTemplatesListEmbedded(t *testing.T) {
	...
	for _, want := range []string{"prompts/research.md", "prompts/plan.md", "prompts/implement.md", "skills/respec/SKILL.md", "skills/respec/agents/openai.yaml", "[embedded]"} {
...
func TestTemplatesEjectAndListShowsOverride(t *testing.T) {
	...
	// Eject a single prompt.
	if _, err := runRespecErr(t, "templates", "eject", "research", "--force=false"); err != nil {
		t.Fatalf("eject: %v", err)
	}
	ejected := filepath.Join(tdir, "prompts", "research.md")
	if _, err := os.Stat(ejected); err != nil {
		t.Fatalf("ejected file missing: %v", err)
	}

	// list --json should now report research.md sourced from the override dir.
	...
	found := false
	for _, i := range infos {
		if i.Name == "prompts/research.md" {
			found = true
			if i.Source != tdir {
				t.Errorf("research.md source = %q, want %q", i.Source, tdir)
			}
		}
	}
	if !found {
		t.Error("research.md not in list output")
	}
	...
	if _, err := runRespecErr(t, "templates", "eject", "research", "--force"); err != nil {
